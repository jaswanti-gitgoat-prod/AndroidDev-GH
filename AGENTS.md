...
- **High risk - report for review outside source code:** Password hashing changes, encryption key modifications, database schema changes. Do not add source-code comments solely to flag these issues
...
5. **Report high-risk issues outside code** - Mention critical security issues that need careful planning in the assistant response, PR summary, or issue tracker; do not modify source files only to annotate unrelated vulnerabilities
...
- **Append known authorship metadata** - When available, append metadata suffixes after the explanation in this order: `| Agent: [agent name] | Model: [model name] | Prompted By: [full name or git username]`
...
- **Append optional metadata outside the rule ID** - When known, append suffixes after the explanation: `Agent: [agent name]`, `Model: [model name]`, and `Prompted By: [full name or git username]`
...
- **Skip unknown metadata** - If the agent name, model name, or prompting user identity is unavailable, omit that suffix instead of guessing
- **Skip generic prompting identities** - Omit `Prompted By` when the available value is generic, automated, or non-identifying, such as `system`, `user`, `assistant`, `unknown`, `anonymous`, `automation`, or a service-account placeholder
- **Avoid email addresses** - For `Prompted By`, use non-email identity provider context such as full name, display name, or git username; do not include email addresses
- **Example**: `Agentic Rule (AUTH_TYPESCRIPT_CREATE_HASHUSERPASSWORD): Using bcrypt with 12 rounds for secure password hashing | Agent: Cursor | Model: GPT-5.5 | Prompted By: mike-roservice`
...
4. Use it in your code comment exactly as shown, then append any known `Agent` and `Model` suffixes, plus `Prompted By` only when it identifies a real prompting user
...
- **Added meaningful authorship metadata** - Included `Agent`, `Model`, and `Prompted By` suffixes when that context is available and identifying, without using email addresses or generic prompting identities
...
- **Reported high-risk issues outside code** - Identified critical security problems requiring careful planning without adding source-code-only annotations
...
- **Post-quantum readiness** - when adding or modifying public-key cryptography, treat data as needing long-term protection if it includes secrets, credentials, regulated data, customer PII, backups, archives, or persisted encrypted records, or if retention is unclear; for those cases and for long-lived signatures or custom key exchange, use approved NIST-standardized post-quantum or hybrid schemes (ML-KEM, ML-DSA, SLH-DSA) through maintained provider-backed libraries, or document a security-reviewed exception when platform or legacy ecosystem support is not mature | Rule ID:`[ARNIE_CRYPTO_POST_QUANTUM_READINESS]`
